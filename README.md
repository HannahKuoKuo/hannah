...
- [ ] 圖片輪播：目前只有 1 張（頂樓交誼廳實景），見下方「圖片輪播」說明
...
**圖片輪播**
- Lumina 與 FOUND 都做了 6 張輪播（左右箭頭 + 下方圓點切換，純 CSS/JS 實作，無外部套件）
- Panorama 目前只有 1 張——它的原始檔案在 Google Drive 資料夾裡幾乎都超過 8MB，這個環境的下載工具對這麼大的檔案不穩定，
  重試多次都失敗（10MB 以上會直接被工具擋掉）。只有 1 張圖時輪播會自動隱藏箭頭跟圓點，不會顯示壞掉的介面，
  但建議之後幫 Panorama 補到 5-6 張。你可以之後再讓我重試下載，或是把圖片壓縮小一點（例如轉存成 5MB 以下的 JPEG）後
  放到同一個 Drive 資料夾，我再抓一次通常會成功
- 三個資料夾裡都還有更多張沒用到（Panorama 9 張、Lumina 11 張、FOUND 18 張），如果想換掉某張或調整順序，
  跟我說檔名或描述即可
