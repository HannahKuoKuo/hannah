...
assets/      圖片素材（已放入三案各一張實景圖，已壓縮至網頁適用大小）
```
...
**建案圖片**
- 目前三案各放了一張你 Google Drive 資料夾裡的實景照（已壓縮到約 130–180KB／張，寬度 1600px，適合網頁載入速度）
- 你的資料夾裡其實還有很多張（Panorama 9 張、Lumina 11 張、FOUND 18 張），這裡先每案選一張代表圖上架；
  如果想放更多張（例如做成小型 gallery、輪播圖，或每個戶型配對應的實景圖），跟我說要哪幾張（檔名或描述）即可
- FOUND 目前這張是品牌發表活動的展示空間照（含 HEXA / FOUND 攤位背板），若想換成更明確對應「倉庫／自儲／辦公室」產品本身的照片，可以指定資料夾內其他張
